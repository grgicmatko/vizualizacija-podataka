...
function clicked(event, d) {
  ...
  // Update fill color of all countries
  ...
  // Adjust layout for country info panel
...
function showCountryInfo(country) {
  ...
  if (topSports.length > 0) {
    ...
    const value = calculateUniqueMedalsBySport(country) + 1;
    ...
    // Append a div for the scale
    const scaleDiv = slider.append("div").attr("class", "scale");

    slider.append("p").html(`${least[0]}<br>${least[1] - 1}`);
    ...
    // Append the scale visualization
    const scaleVisualization = scaleDiv
      .append("div")
      .attr("class", "scale-visualization");

    // Append a dot representing the value
...
function findCountryWithLeastUniqueMedals() {
  ...
  olympicData.forEach((d) => {
    const country = normalizeCountryName(d.Team);
    const eventKey = `${d.Year}-${d.Sport}-${d.Event}`;

    if (!uniqueMedalsByCountry[country]) {
      uniqueMedalsByCountry[country] = {
        events: new Set(),
        sports: new Set(),
      };
    }

    uniqueMedalsByCountry[country].events.add(eventKey);
    uniqueMedalsByCountry[country].sports.add(d.Sport);
  });

  let leastUniqueMedalsCount = Infinity;
  let leastUniqueMedalsCountry = null;

  Object.entries(uniqueMedalsByCountry).forEach(
    ([country, { events, sports }]) => {
      const uniqueEventsCount = events.size;
      const uniqueSportsCount = sports.size;
      const uniqueMedalsCount = uniqueEventsCount + uniqueSportsCount;

      if (uniqueMedalsCount > 0 && uniqueMedalsCount < leastUniqueMedalsCount) {
...
function calculateUniqueMedalsBySport(country) {
  const countryName = country.properties.name;
  const filteredData = filterDataByCountry(olympicData, countryName);
  const aggregatedData = aggregateMedalsBySport(filteredData);
  const topSports = getTopSports(aggregatedData, -1);

  let sum = 0;
  topSports.forEach((value) => {
    sum += value.count;
  });
  return sum;
}
